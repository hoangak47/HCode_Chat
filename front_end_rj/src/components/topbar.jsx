...
function Topbar({ infoRoom, setOpen, dispatch, openProfile, userIsFriend }) {
        ...
        <div className="flex items-center justify-between h-16 px-4 py-10 border-b border-gray-200">
            <div className="flex items-center">
                ...
                <div className="ml-3">
                    {infoRoom?.isGroupChat ? (
                        <>
                            <p className="text-base font-semibold text-gray-900 text-line-1">{infoRoom?.name}</p>
            ...
            <div className="flex items-center">
                {buttonActionMessage.map((item) => (
                    <Tooltip key={item.key} placement="top" title={item.label} color="#7a7f9a" mouseLeaveDelay={0}>
                        <div
